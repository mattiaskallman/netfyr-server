...
pub async fn login(
    ...
    if username.is_empty() || body.password.is_empty() {
        ...
        return Err(ApiError::bad_request(crate::i18n::username_password_required(lang)));
    }
    ...
    let outcome = state
        ...
        .call(move |conn| {
            let row: Option<(i64, String, i64, i64, Option<i64>)> = conn
                .query_row(
                    "SELECT id, password_hash, disabled, failed_attempts, locked_until
            ...
            let Some((user_id, hash, disabled, failed, locked_until)) = row else {
    ...
    match outcome {
        Ok((user_id, token)) => {
            audit_login(&state, &body.username.trim().to_lowercase(), "login_ok", &ip).await;
            let me = me_view(&state, user_id).await?;
            let cookie = auth::session_cookie(&token, state.session_hours, state.secure_cookies);
            Ok((
        ...
        Err(reject) => {
            audit_login(&state, &body.username.trim().to_lowercase(), "login_fail", &ip).await;
            let lang = crate::i18n::load_db(&state.db).await;
            match reject {
                ...
                LoginReject::BadCreds => {
                    Err(ApiError::unauthorized(crate::i18n::login_fail(lang)))
                }
            }
        }
    }
...
pub async fn logout(
    ...
    Ok(([(header::SET_COOKIE, auth::clear_cookie(state.secure_cookies))], Json(())).into_response())
}
...
pub async fn change_password(
    ...
    if let Err(msg) = auth::validate_password(&body.new_password, crate::i18n::load_db(&state.db).await) {
